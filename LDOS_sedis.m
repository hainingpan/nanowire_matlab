function LDOS_sedis(mu,Delta,gamma,vc,muVar,mulist,dim)
% mu=1;
% Delta=.2;
alpha_R=5;
delta=1e-3;
Vzlist=linspace(0,2.048,11);
energylist=linspace(-.3,.3,11);
...
parfor i=1:lenVz
    ...
    Vz=Vzlist(i);
    energy=energylist(j);
    ldosmap(i,j,:)=ldosall_sedis(mu,Delta,Vz,alpha_R,gamma,vc,dim,mulist,energy,delta);    
    end
end
