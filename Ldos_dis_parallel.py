...
def main():
    ...
    if rank==0:
        ...
        fig,ax=plt.subplots()
        ax.pcolormesh(np.linspace(0,parameters['Vzmax'],tot_vz),np.linspace(-parameters['Vbiasmax'],parameters['Vbiasmax'],tot_energy),recvbuf_DOS.T)
        ...
        fig,ax=plt.subplots()
        ax.pcolormesh(np.linspace(0,parameters['Vzmax'],tot_vz),np.linspace(-parameters['Vbiasmax'],parameters['Vbiasmax'],tot_energy),recvbuf_LDOS_L.T)
        ax.set_xlabel('Vz(meV)')
        ax.set_ylabel(r'$V_\mathrm{bias}$ (meV)')
        ax.set_title('LDOS(Left)')
        fig.savefig(fn+'_LDOS_L.png')

        fig,ax=plt.subplots()
        ax.pcolormesh(np.linspace(0,parameters['Vzmax'],tot_vz),np.linspace(-parameters['Vbiasmax'],parameters['Vbiasmax'],tot_energy),recvbuf_LDOS_M.T)
        ax.set_xlabel('Vz(meV)')
        ax.set_ylabel(r'$V_\mathrm{bias}$ (meV)')
        ax.set_title('LDOS(Middle)')
        fig.savefig(fn+'_LDOS_M.png')

        fig,ax=plt.subplots()
        ax.pcolormesh(np.linspace(0,parameters['Vzmax'],tot_vz),np.linspace(-parameters['Vbiasmax'],parameters['Vbiasmax'],tot_energy),recvbuf_LDOS_R.T)
        ax.set_xlabel('Vz(meV)')
        ax.set_ylabel(r'$V_\mathrm{bias}$ (meV)')
        ax.set_title('LDOS(Right)')
