...
function [dosmap,rev]=spec_seqd_sp(a,mu,delta,alpha,gamma,vc,mumax,l0,dim,period)
% a=1;
...
parfor i=1:length(vzlist)
    vz=vzlist(i);
    disp(i);    
    dos=arrayfun(@(w) dosseqd(a,mu,delta,vz,alpha,gamma,vc,mumax,l0,dim,w,1e-3,period),enlist);
    [~,loc]=findpeaks(dos);
    init=enlist(loc);
%     dosmap2(i,:)=dos;
